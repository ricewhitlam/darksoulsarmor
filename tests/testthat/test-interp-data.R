## get.interp.data blends the unupgraded (*_00) and fully upgraded (*_10) tables by upgrade
...
test_that("get.interp.data reproduces the unupgraded table at level 0", {
    interp <- darksoulsarmor:::get.interp.data(head.data_00, head.data_10, 0, 0)
    interp <- interp[order(ARMOR)]

    reg.interp <- interp[UPGRADE_TYPE == "Regular"]
    reg.base <- head.data_00[UPGRADE_TYPE == "Regular"][order(ARMOR)]
    ...
    twink.base <- head.data_00[UPGRADE_TYPE == "Twinkling"][order(ARMOR)]
...
test_that("get.interp.data reproduces the fully upgraded table at max level", {
    interp <- darksoulsarmor:::get.interp.data(head.data_00, head.data_10, 10, 5)
    interp <- interp[order(ARMOR)]

    reg.interp <- interp[UPGRADE_TYPE == "Regular"]
    reg.max <- head.data_10[UPGRADE_TYPE == "Regular"][order(ARMOR)]
    expect_equal(reg.interp$PHYS_DEF, reg.max$PHYS_DEF)
    expect_equal(reg.interp$BLEED_RES, reg.max$BLEED_RES)

    twink.interp <- interp[UPGRADE_TYPE == "Twinkling"]
    twink.max <- head.data_10[UPGRADE_TYPE == "Twinkling"][order(ARMOR)]
...
test_that("get.interp.data leaves UPGRADE_TYPE == 'None' pieces unaffected by level", {
    at.zero <- darksoulsarmor:::get.interp.data(head.data_00, head.data_10, 0, 0)
    at.max <- darksoulsarmor:::get.interp.data(head.data_00, head.data_10, 10, 5)

    none.zero <- at.zero[UPGRADE_TYPE == "None"][order(ARMOR)]
    none.max <- at.max[UPGRADE_TYPE == "None"][order(ARMOR)]
    none.base <- head.data_00[UPGRADE_TYPE == "None"][order(ARMOR)]
